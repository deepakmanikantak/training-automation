...
/*
 * the purpose of this class is to show the absolute
 * difference  between primary and secondary diagnol
...
public class DiagnolDiffernceArray {
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 0; i < arr.length; i++) {
                for (int j = 0; j < arr.length; j++) {
                    //logic for the sum of primary diagnol elements
                    if (i == j) {
                        primarySum = primarySum + arr[i][j];
                    }
                    //logic for the sum of secondary diagnol elements
                    if ((i + j) == (n - 1)) {
